...
*/
		for(i=1;i<=gridi-2;i++){
			for(j=1;j<=gridj-2;j++){
				if(i==1 || i==gridi-2 || j==1 || j==gridj-2){
					if(j<gridi-l) omega[0][j]=(v[1][j]-v[0][j])/dx; else omega[gridi-k-1][j]=(v[gridi-k][j]-v[gridi-k-1][j])/dx;
					if(j<l) omega[k][j]=(v[k][j]-v[k-1][j])/dx; else omega[gridi-1][j]=(v[gridi-1][j]-v[gridi-2][j])/dx;
					if(i<k) omega[i][0]=-(u[i][1]-u[i][0])/dy; else omega[i][l]=-(u[i][l+1]-u[i][l])/dy;
					if(i<gridi-k) omega[i][gridj-l-1]=-(u[i][gridj-l-1]-u[i][gridj-l-2])/dy; else omega[i][gridj-1]=-(u[i][gridj-1]-u[i][gridj-2])/dy;
				}
			}
		}
...
void WriteFile(string a,int iter, int is, int ie, int js, int je){
			file.open(a, ios::out | ios::app);

			file<<"TITLE=\"2D\"\nVARIABLES=\"X\",\"Y\",\"psi\",\"omega\",\"u\",\"v\"";
			
			file<<"\nZONE\tT=BLOCK"<<iter<<"\tI="<<ie-is<<",J="<<je-js<<",\tF=POINT\n";
			
			for(j=js;j<=je-1;j++)
				for(i=is;i<=ie-1;i++){
					file<<i*dx<<"\t"<<j*dx<<"\t"<<psi[i][j]<<"\t"<<omega[i][j]<<"\t"<<u[i][j]<<"\t"<<v[i][j]<<endl;
				}
			file.close();
}
...
*/
	...
	Solve(0,43,40,100);
		...
		if(iter%500==0){
			...
			WriteFile("1.plt",iter,39,140,96,140);
			WriteFile("2.plt",iter,97,140,40,100);
			WriteFile("3.plt",iter,39,101,40,100);
			WriteFile("4.plt",iter,0,101,0,44);
			WriteFile("5.plt",iter,0,43,40,100);
		}
